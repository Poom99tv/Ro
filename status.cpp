...
int status_calc_pc_sub(map_session_data* sd, uint8 opt)
{
	...
	struct npc_data *refine_bonus_calculate;

	refine_bonus_calculate = npc_name2id("s_refine_bonus_system");
	if( sd && refine_bonus_calculate ) {
		run_script(refine_bonus_calculate->u.scr.script, 0, sd->bl.id, refine_bonus_calculate->bl.id);
		if( !calculating )
			return 1;
	}
...
TIMER_FUNC(status_change_timer){
	...
	case SC_AUTOATTACK:
		if (--(sce->val4) > 0) {
					...
					if(sd->aa.itempick_id){
						...
						if(fitem_bl){
							...
							if(!check_distance_bl(&sd->bl, fitem_bl, 2))
								...
								if(!sd->aa.last_pickup || DIFF_TICK(gettick(), sd->aa.last_pickup) > 0){
									pc_takeitem(sd,fitem);
									sd->aa.last_pickup = gettick() + 500;
								}
							}
						}
					}
					...
					if(battle_config.feature_autoattack_attackskill){
						if(last_tick >= sd->aa.skill_cd && sd->aa.autoattackskills.size() > 0){
							...
							for(auto &itAutoattackskills : skills_temp_list){
								short random_skill = rand() % 100;
								if(last_tick >= sd->aa.skill_cd && last_tick >= itAutoattackskills.last_use && random_skill <= 25){ // 25% luck of using skill
									if(itAutoattackskills.is_active 
										...
										itAutoattackskills.last_use = last_tick + pc_get_skillcooldown(sd, itAutoattackskills.skill_id, itAutoattackskills.skill_lv );
									}
									if(itAutoattackskills.last_use > sd->aa.skill_cd)
										sd->aa.skill_cd = itAutoattackskills.last_use;
									if(sd->aa.skill_cd < (last_tick + pc_get_skillcooldown(sd, itAutoattackskills.skill_id, itAutoattackskills.skill_lv )) )
										sd->aa.skill_cd = last_tick + pc_get_skillcooldown(sd, itAutoattackskills.skill_id, itAutoattackskills.skill_lv );
								}
							}
						}
					}
			...
			sce->timer = add_timer(300 + tick, status_change_timer, bl->id, data);
			return 0;
		}
		break;
	case SC_MAXIMIZEPOWER:
...
unsigned int aa_check_item_pickup_onfloor(map_session_data *sd){
	...
	sd->aa.itempick_id = 0;
	int itemid = 0;
	for (int i = 0; i < battle_config.feature_autoattack_pdetection; i++){
...
void autoattack_clear(map_session_data *sd)
{
	...
	std::vector<uint16> atk_skill_lists = {};
	std::vector<uint16> sup_skill_lists = {};
	std::vector<uint16> heal_skill_lists = {};
	std::vector<uint16> sd_skill_lists = {};

	if(sd->aa.autoattackskills.size()){
		for(const auto &attack_skill : sd->aa.autoattackskills){
			atk_skill_lists.push_back(attack_skill.skill_id);
		}
	}

	if(sd->aa.autobuffskills.size()){
		for(const auto &buff_skill : sd->aa.autobuffskills){
			sup_skill_lists.push_back(buff_skill.skill_id);
		}
	}

	if(sd->aa.autoheal.size()){
		for(const auto &heal_skill : sd->aa.autoheal){
			heal_skill_lists.push_back(heal_skill.skill_id);
		}
	}

	for(int i=0;i<MAX_SKILL;i++){
		if(sd->status.skill[i].id > 0 && sd->status.skill[i].lv > 0 && sd->status.skill[i].flag == SKILL_FLAG_PERMANENT ){
			std::shared_ptr<s_skill_db> skill = skill_db.find(sd->status.skill[i].id);
			if (skill)
				sd_skill_lists.push_back(sd->status.skill[i].id);
		}
	}

	for(const auto &attack_skill : atk_skill_lists){
		if(std::find(sd_skill_lists.begin(), sd_skill_lists.end(), attack_skill) == sd_skill_lists.end()){
	...
	for(const auto &buff_skill : sup_skill_lists){
		if(std::find(sd_skill_lists.begin(), sd_skill_lists.end(), buff_skill) == sd_skill_lists.end()){
	...
	for(const auto &heal_skill : heal_skill_lists){
		if(std::find(sd_skill_lists.begin(), sd_skill_lists.end(), heal_skill) == sd_skill_lists.end()){
